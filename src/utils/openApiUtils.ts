...
export class ParsedPathItemObject {
	...
	private processPathParameters(): void {
			...
			if ('name' in parameterObject) {
				if (parameterObject.in == 'path' && parameterObject.name) {
					if (parameterObject.schema && 'type' in parameterObject.schema) {
						...
						// If parameter was defined in path but doesn't have ObjectSchema
...
export function parsePathPattern(path: string): RegExp {
	let safePathRegexStr = path
		.replace(/{(.+?)}/g, function(match: string, varName: string) {
			return `(?<${varName}>[^\\/]+)`;
		})
		.replace(/\//, '\\$&');
...
export function parsePathsObject(pathsObject: OpenAPIV3.PathsObject): ParsedPathItemObject[] {
	...
	for (const pathPattern in pathsObject) {
		if (!pathsObject.hasOwnProperty(pathPattern)) continue;
...
export function findPathMatch(path: string, parsedPathItemObjects: ParsedPathItemObject[]): PathMatch | undefined {
